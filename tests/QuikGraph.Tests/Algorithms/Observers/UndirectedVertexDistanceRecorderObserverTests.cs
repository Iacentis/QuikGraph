...
namespace QuikGraph.Tests.Algorithms.Observers
{
    ...
    internal sealed class UndirectedVertexDistanceRecorderObserverTests : ObserverTestsBase
    {
        ...
        public void Constructor_Throws()
        {
            ...
            Assert.Throws<ArgumentNullException>(() =>
                new UndirectedVertexDistanceRecorderObserver<int, Edge<int>>(null, DistanceRelaxers.ShortestDistance,
                    new Dictionary<int, double>()));
            Assert.Throws<ArgumentNullException>(() =>
                new UndirectedVertexDistanceRecorderObserver<int, Edge<int>>(_ => 1.0, null,
                    new Dictionary<int, double>()));
            Assert.Throws<ArgumentNullException>(() =>
                new UndirectedVertexDistanceRecorderObserver<int, Edge<int>>(_ => 1.0,
                    DistanceRelaxers.ShortestDistance, null));
            Assert.Throws<ArgumentNullException>(() =>
                new UndirectedVertexDistanceRecorderObserver<int, Edge<int>>(null, null,
                    new Dictionary<int, double>()));
            Assert.Throws<ArgumentNullException>(() =>
...

            {
                ...
                using (recorder.Attach(dfs))
                {
                    ...
                    CollectionAssert.AreEqual(
                        new Dictionary<int, double> { [1] = 0, [2] = 1, [3] = 3, [4] = 2 },
                        recorder.Distances);
                }
            }
        }
